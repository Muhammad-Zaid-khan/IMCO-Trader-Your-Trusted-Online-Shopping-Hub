  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 lg:px-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-semibold text-primary-six">{currentCategoryName}</h2>
          <p className="text-sm text-gray-500 mt-1">
            {/* Categories • {categoriesWithSubcategories[currentCategoryIndex]?.subcategories?.join(' • ')} */}
          </p>
        </div>
        ...
        {displayedProducts.length > productsPerPage && (
          <>
            ...
            <button
              ...
              disabled={currentProductIndex >= displayedProducts.length - productsPerPage}
              className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-lg border flex items-center justify-center transition-all ${
                currentProductIndex >= displayedProducts.length - productsPerPage
                  ? 'opacity-50 cursor-not-allowed' 
